...
import fs__default, { constants } from 'node:fs/promises';
...
const wslDrivesMountPoint = (() => {
	...
	return async function () {
		...
		try {
			await fs__default.access(configFilePath, constants.F_OK);
		...
		const configContent = await fs__default.readFile(configFilePath, {encoding: 'utf8'});
...
const baseOpen = async options => {
	...
	if (platform === 'darwin') {
		...
		if (app) {
			...
			try {
				await fs__default.access(localXdgOpenPath, constants.X_OK);
...
//# sourceMappingURL=index-k7Vesebj.js.map
